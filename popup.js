...
// ==========================
// Worked time with 09:00 clamp
// ==========================
function computeWorkedFromPairsClamped(pairs, nowMin) {
  // Sum each IN→OUT segment but only within [09:00, nowMin]
  // If OUT missing, assume "now"
  let totalSec = 0;
  for (const p of pairs) {
    ...
    // If only OUT exists (should be rare), skip
    if (inMin == null && outMin != null) continue;

    // No OUT → treat OUT = now
    if (outMin == null) outMin = nowMin;

    // Clip to [09:00, now]
    const start = Math.max(inMin, OFFICE_START_MIN);
    const end   = Math.min(outMin, nowMin);
    if (end > start) {
      totalSec += (end - start) * 60;
    }
  }
  return totalSec;
}
...
async function readWorkedFromPage() {
  ...
  try {
    ...
    // Weekly (uses Zoho's per-day worked as-is; weekends treated as leave)
...
saveTargetBtn.addEventListener('click', async () => {
  const hhmm = targetInputEl.value;          // "HH:MM"
  const targetSeconds = hhmmToSeconds(hhmm);
...
(async function init() {
  ...
  // Pre-fill time input
  targetInputEl.value = secondsToHHMM(targetSeconds);
  setTargetDisplay(targetSeconds);
